...
class SetupScreen(Screen):
    ...
    def refresh_gist_state(self):
        self.gist_enabled = gist_sync.is_enabled()
        if not gist_sync.is_configured():
            self.gist_code_text = gist_sync.NOT_CONFIGURED_MESSAGE
    ...
    def start_device_flow(self):
        ...
        def work():
            try:
                ...
                log.error(f"setup screen: device flow transport failed ({e})")
                self._post_code_cleared()
    ...
    def open_revoke_page(self):
        ...
        try:
            ...
            log.warning(f"setup screen: cannot open {gist_sync.REVOKE_URL} ({e})")
    ...
    def restore_from_gist(self):
        ...
        def work():
            try:
                ...
                log.error(f"setup screen: gist list failed ({e})")
    ...
    def select_restore_gist(self, gist_id: str):
        ...
        def work():
            try:
                ...
                log.error(f"setup screen: gist fetch failed ({e})")
                self._post_status("Could not reach GitHub.")
                return
