/* Mode watch -- diagnostic probe, schema 4.
 *
 ...
 * == 0 and counts the refusals. With the F1/F2 firmware fixes and the UI's
 * sync-first teardown on this branch, that counter is EXPECTED to stay at
 * zero; a nonzero count means some path still leaves sync armed across a
 * disengage and is a finding in its own right (and a divergence datapoint).
 ...
 *
 * Field meanings for schema 4 -- NOT schema 2's or 3's; check diagSchema:
 *
 ...
 *   diagNetCounts     latch suppressions (32-bit, cumulative). EXPECT 0
 *   diagEndReason     1 once any suppression has been seen, else 0
 *   diagTrace[0]      servoMode at the most recent suppression
 *   diagTrace[1]      elsStop.active at the most recent suppression
...
/* Schema 3's intervention, carried forward verbatim in behavior; only the
 * event record moved (into diagTrace[0..1]), because this schema's
 * capture/settle fields carry modes. See els_diag_disengage_latch.h for the
 * full reasoning — suppression instead of observation is what makes the
 * dangerous outcome impossible while the counter still proves whether the
 * condition arises. */
