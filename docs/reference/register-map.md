<!-- GENERATED by tools/genregs.py from registers/*.yaml. DO NOT EDIT. -->

# Register map (protocolVersion 11)

The generated structs of `rampsSharedData_t`, in the order they sit in it; 246 registers in all. Registers 0..7 (the ISR timing counters ahead of `servo`) are declared by hand in `fw/Core/Inc/Ramps.h` and are not described here.

## `servo_t` — `rampsSharedData_t.servo`

From `registers/servo.yaml`. 18 registers at 8..25.

### `all` — 18 registers (servo 0..17, absolute 8..25)

The whole struct. Servo configuration and live ramp state; the tick-rate mirrors of the live values are in fastData_t.

| reg | abs | type | field | access | notes |
|----:|----:|------|-------|--------|-------|
| 0 | 8 | `float` | `maxSpeed` | sw_write | SW write: maximum step rate (steps/s); clamped to 100000 by firmware |
| 2 | 10 | `float` | `currentSpeed` | ro_firmware | READ-ONLY (firmware-owned): live ramp speed, managed by ramp algorithm |
| 4 | 12 | `float` | `jogSpeed` | sw_write | SW write: jog target speed (steps/s); negative = reverse |
| 6 | 14 | `float` | `acceleration` | sw_write | SW write: ramp acceleration (steps/s²) |
| 8 | 16 | `int32` | `stepsToGo` | sw_write | SW write: remaining steps for indexing move; firmware decrements toward zero |
| 10 | 18 | `uint32` | `destinationSteps` | sw_write | SW write: absolute destination step count for indexing mode |
| 12 | 20 | `uint32` | `currentSteps` | ro_firmware | READ-ONLY (firmware-owned): step counter incremented/decremented by ISR |
| 14 | 22 | `uint32` | `desiredSteps` | ro_firmware | READ-ONLY (firmware-owned): accumulated target steps, driven by sync/ramp |
| 16 | 24 | `int16` | `servoDir` | sw_write | SW write: ±1, default +1 (no inversion); applied to DIR pin in ISR |
| 17 | 25 | — | *(implicit tail pad)* | — | sizeof rounds up to 4-byte alignment |

## `input_t` — `rampsSharedData_t.scales`

From `registers/input.yaml`. `scales[4]`: 12 registers per element at 26 + 12·i, 26..73 in all. The `abs` column is element 0.

### `all` — 12 registers (input 0..11, absolute 26..37)

The whole struct: one scale's encoder state and its sync configuration.

| reg | abs | type | field | access | notes |
|----:|----:|------|-------|--------|-------|
| 0 | 26 | `uint32` | `timerHandleSlot` | init_only | init-only: index into ramps_timer_handles[]; held as a 4-byte slot id (not a pointer) so the modbus wire layout is identical on STM32 and 64-bit emulator hosts |
| 2 | 28 | `int32` | `position` | ro_firmware | READ-ONLY (firmware-owned): absolute encoder position, updated by ISR |
| 4 | 30 | `int32` | `speed` | ro_firmware | READ-ONLY (firmware-owned): encoder speed (counts/s), updated by updateSpeedTask |
| 6 | 32 | `int32` | `syncRatioNum` | sw_write | SW write: sync ratio numerator/denominator (output steps per input count) |
| 8 | 34 | `int32` | `syncRatioDen` | sw_write | SW write: sync ratio numerator/denominator (output steps per input count) |
| 10 | 36 | `uint16` | `syncEnable` | sw_write | SW write: 0 = sync disabled, non-zero = sync enabled for this scale |
| 11 | 37 | `int16` | `scaleDir` | sw_write | SW write: ±1, default +1 (no inversion); applied to encoder delta in ISR |

## `fastData_t` — `rampsSharedData_t.fastData`

From `registers/fast_data.yaml`. 30 registers at 74..103.

### `all` — 30 registers (fastData 0..29, absolute 74..103)

The whole struct. Firmware-published mirrors of the live servo and scale state, refreshed by the ISR and updateSpeedTask, plus the servo mode. Read as one request per board tick.
...
| 24 | 98 | `uint32` | `cycles` | ro_firmware | READ-ONLY (firmware-owned): ISR execution time in CPU cycles, updated by updateSpeedTask |
| 26 | 100 | `uint32` | `executionInterval` | ro_firmware | READ-ONLY (firmware-owned): ISR interval in CPU cycles, updated by ISR |
| 28 | 102 | `uint16` | `servoMode` | bidirectional | SW write: 0=disabled, 1=sync/index (also set by firmware), 2=jog |
| 29 | 103 | — | *(implicit tail pad)* | — | sizeof rounds up to 4-byte alignment |

## `elsStop_t` — `rampsSharedData_t.elsStop`

From `registers/els_stop.yaml`. 142 registers at 104..245.
...
| 47 | 151 | `int16` | `stopTriggerOffset` | ro_firmware | READ-ONLY (firmware-owned): the CLAMPED stopOffset actually in effect when this trigger fired, encoder counts (0..ELS_STOP_OFFSET_MAX). So the stop fired at stopPosition - sign(stopDirection) * stopTriggerOffset, and stopTriggerZ is that effective threshold (or just past it). Recorded rather than inferred from what the host last wrote, because the host's write and the trigger race |
...
| 61 | 165 | `int16` | `stopOffset` | sw_write | SW write: stop-overshoot correction in encoder counts. The ISR fires the stop this many counts EARLY (effective threshold = stopPosition - sign(stopDirection) * clamp(stopOffset, 0, ELS_STOP_OFFSET_MAX)); >= 0 fires earlier, negative is treated as 0, above ELS_STOP_OFFSET_MAX (200 counts = 1 mm on elspi) is clamped. The hysteresis clearance is measured from the same effective threshold. Written LIVE by reflex-ui from the approach Z rate; stopPosition stays the exact, overshoot-ignorant target. A SEPARATE 16-bit register rather than a rewritten stopPosition because Modbus FC16 copies a 32-bit value one 16-bit half at a time and the ISR could see it torn; a single 16-bit read is atomic. 0 = no correction (and the reset value) |
