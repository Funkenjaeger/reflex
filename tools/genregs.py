...
def emit_c(doc, items, groups, total_regs, consts):
    ...
    for i in items:
        if i["group"] != cur:
            ...
            line = "   *"
            for word in c_comment_safe(gdoc).split():
    ...
    for i in items:
        if i["pad"]:
            continue
        L.append(f"static_assert(offsetof(elsStop_t, {i['name']}) == {i['byte_off']}, "
                 f"\"{i['name']} moved: schema says register {i['byte_off'] // 2}\");")
    L.append(f"static_assert(sizeof(elsStop_t) == {total_regs * 2}, "
             f"\"elsStop_t is not {total_regs} registers\");")
    for grp in [g["group"] for g in doc["layout"]]:
        s = groups[grp]
        budget = next(g for g in doc["layout"] if g["group"] == grp)["budget_registers"]
        L.append(f"static_assert(ELS_STOP_{grp.upper()}_REG_COUNT <= {budget}, "
...
def emit_py(doc, items, groups, total_regs):
    ...
    # compiler has already agreed to via _Static_assert.
    #
    ...
    for i in items:
        if i["pad"]:
            pad_n += 1
            L.append(f"  uint16_t _pad{pad_n};")
            continue
...
def check_external_consumers(doc, items):
...
def emit_json(doc, items, total_regs):
    ...
    return json.dumps({
        ...
        # Valid because elsStop is the LAST member of rampsSharedData_t, which
        # the C header states and the contract test enforces via sizeof.
        #
        ...
        "struct_registers": base + total_regs,
...
def main():
    ...
    check_external_consumers(doc, items)
